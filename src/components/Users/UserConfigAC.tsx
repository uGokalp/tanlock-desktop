...
import mediumCrud from "@/db/tables/medium-crud"
...
const UserConfigAC = ({ medium }: Props) => {
  const [deleteModal, setDeleteModal] = useState(false)

  const router = useRouter()
  const { register, handleSubmit, getValues } = useForm<Medium>({
    defaultValues: medium,
  })

  const updateUser = useMutation({
    mutationFn: async (medium: Medium) => {
      return await mediumCrud.update(medium)
    },
    onSuccess: () => {
      void queryClient.refetchQueries(["db-mediums"])
    },
  })

  const deleteUser = useMutation({
    mutationFn: async (medium: Medium) => {
      return await mediumCrud.delete(medium)
    },
    onSuccess: () => {
      void queryClient.refetchQueries(["db-mediums"])
    },
  ...
  const onDelete = () => {
    const medium = getValues()
    deleteUser.mutate(medium)
    onCancel()
  }
  const onCancel = () => {
